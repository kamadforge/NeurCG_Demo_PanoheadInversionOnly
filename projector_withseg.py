...
def project(
    ...
    delta_c = G.t_mapping(torch.from_numpy(np.mean(z_samples, axis=0, keepdims=True)).to(device), camera[:1],
    ...
    for step in range(num_steps):
        ...
        ws = w_opt + w_noise
        synth_images = G.synthesis(ws, c=camera, noise_mode='const')['image']
...
def run_projection(
    ...
    if target_img is not None:
        ...
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=target_img,
        ...
        target_fname = dataset._path + "/" + dataset._image_fnames[idx]
        cond = torch.from_numpy(dataset._get_raw_labels()[idx:idx + 1]).to(device)
        # import ipdb; ipdb.set_trace()
        if cond.shape[1] == 0:
            pose = LookAtPoseSampler.sample(
                3.14 / 2, 3.14 / 2, torch.tensor(G.rendering_kwargs.get('avg_camera_pivot', (0, 0, 0))), radius=2.7)
            focal_length = 4.26339
            intrinsics = torch.tensor([[focal_length, 0, 0.5], [0, focal_length, 0.5], [0, 0, 1]])
            c = torch.cat([pose.reshape(-1, 16), intrinsics.reshape(-1, 9)], 1).to(device)
            cond = c.repeat((cond.shape[0], 1))
        print(f"projecting: [{idx}] {target_fname}")
        print(f"camera matrix: {cond.shape}")
    # Load target image.
    ...
    projected_w_steps, cond = project(
        G,
    ...
    if save_video:
        ...
        for i, projected_w in enumerate(projected_w_steps[::2]):
            if i % 2 == 1:
                continue
            synth_image = G.synthesis(projected_w.unsqueeze(0).to(device), c=cond, noise_mode='const')['image']
        ...
        for i, G_new in enumerate(G_steps):
            if i % 2 == 1:
                continue
            G_new.to(device)
            synth_image = G_new.synthesis(projected_w_steps[-1].unsqueeze(0).to(device), c=cond, noise_mode='const')[
                'image']
            synth_image = (synth_image + 1) * (255 / 2)
            synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
            video.append_data(np.concatenate([target_uint8, synth_image], axis=1))
    ...
    synth_image = G_final.synthesis(projected_w.unsqueeze(0).to(device), c=cond, noise_mode='const')['image']
    synth_image = (synth_image + 1) * (255 / 2)
    synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
    ...
    if shapes:
        ...
        with tqdm(total=samples.shape[1]) as pbar:
            with torch.no_grad():
                while head < samples.shape[1]:
                    torch.manual_seed(0)
                    sigma = G_final.sample_mixed(samples[:, head:head + max_batch],
                                                 transformed_ray_directions_expanded[:, :samples.shape[1] - head],
                                                 projected_w.unsqueeze(0).to(device), truncation_psi=1,
        ...
        if output_ply:
            ...
            convert_sdf_samples_to_ply(np.transpose(sigmas, (2, 1, 0)), [0, 0, 0], 1,
                                       os.path.join(outdir, 'geometry.ply'), level=10)
        else:  # output mrc
            with mrcfile.new_mmap(os.path.join(outdir, 'geometry.mrc'), overwrite=True, shape=sigmas.shape,
                                  mrc_mode=2) as mrc:
                mrc.data[:] = sigmas
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="project real image into model")

    parser.add_argument('--network', dest='network_pkl', required=True, help='Network pickle filename')
    parser.add_argument('--target_img', required=True, metavar='FILE|DIR', help='Target image folder')
    parser.add_argument('--target_seg', dest='target_seg', required=False, metavar='FILE|DIR',
                        help='Target segmentation folder')
    parser.add_argument('--idx', type=int, default=0, metavar='FILE|DIR', help='index from dataset')
    parser.add_argument('--num_steps', type=int, default=500, help='Number of optimization steps')
    parser.add_argument('--num_steps_pti', type=int, default=500, help='Number of optimization steps for pivot tuning')
    parser.add_argument('--seed', type=int, default=666, help='Random seed')
    parser.add_argument('--save_video', type=bool, default=True, help='Save an mp4 video of optimization progress')
    parser.add_argument('--outdir', required=True, metavar='DIR', help='Where to save the output images')
    parser.add_argument('--fps', type=int, default=30, help='Frames per second of final video')
    parser.add_argument('--shapes', type=bool, default=False, help='Gen shapes for shape interpolation')

    args = parser.parse_args()

    run_projection(network_pkl=args.network_pkl,
